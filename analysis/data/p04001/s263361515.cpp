...
long long solve11(long long p, int q, long long offset); // 再起全探索(探索の実体)
...
long long solve1(long long var)
{
	...
	rtn = solve11(var, std::pow(10, d-1), 0);

	return rtn;
}

long long solve11(long long p, int q, long long offset)
{
	long long rtn = 0;
	...
	if(q == 1) {
		...
		// (1) '+'を挿入する
		rtn += solve11(p%q, q/10, offset + p/q);

		// (2) '+'を挿入しない
		rtn += solve11(p, q/10, offset);
	}

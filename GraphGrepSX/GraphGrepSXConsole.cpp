...
start_p=start_time();
	...
	// ADDED: Print on stdout the LabelMap of the tree.
	std::cout << "##START-LABEL-MAP##" << std::endl;
	LabelMap myLabelMap = ggsx_index._labelMap;
	for(const auto& elem : myLabelMap){
   		std::cout << elem.second << " " << elem.first << "\n";
	}
	std::cout << "##END-LABEL-MAP##" << std::endl;
	// END ADDITION
	std::ofstream os;
	...
	std::cout<<"Saving index on file : "<< topts.db_file << ".index.xml ...\n";
	std::ofstream xml_os;
	xml_os.open((topts.db_file + ".index.xml").c_str(), std::ios::out);
	if(!xml_os.is_open() || xml_os.bad()){
		std::cout<<"Error on opening output file : "<<topts.db_file<<".index.xml \n";
		exit(1);
	}
	ggsx_index._ocpTree.print_xml(xml_os);
	xml_os.flush();
	xml_os.close();
	// END ADDITION
